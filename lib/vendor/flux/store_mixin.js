...
export default {
  ...
  getInitialState() {
    return this.getStateFromStores.call(this)
  },
  ...
  componentDidMount() {
    ...
    for (let key in stores) {
      dispatcher.get(stores[ key ]).addChangeListener( this.handleStoreChanged )
    }
  },
  ...
  componentWillUnmount() {
    let stores = this.constructor.stores
    for (let key in stores) {
      dispatcher.get(stores[ key ]).removeChangeListener( this.handleStoreChanged )
    }
  },
